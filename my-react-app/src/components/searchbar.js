...
export default function FindPeople() {
  ...
  useEffect(() => {
    console.log('find-products function component mounted..!');
    ...
    (async () => {
      try {
        ...
        console.log('Error in useEffect method in find-product:', error);
      }
    })();
  }, []);
    ...
    (async () => {
      try {
        if (searchField) {
          ...
          console.log("Inside 'else loop' in the useEffect POST req in find-people");
          const { data } = await axios.get('/find/users.json');
          setUsers(data);
        }
      } catch (error) {
  ...
  return (
    <>
      <div className="find-product-maincontainer">
        <h3>Search here</h3>
        <input className="search-input" onChange={onChange} placeholder=" Jeans Slim Fit black" />
        <br />
        <div className="found-product-grid">
          {users &&
              ...
              return (
                <div className="newusers-photo" key={index}>
                  <Link to={`/user/${user.id}`}>
                    <img className="new-users-photo" src={user.img_url || 'default-user.jpeg'} />
